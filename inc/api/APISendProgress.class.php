    ...
    public function execute($QUERY = array())
    {
        ...
        foreach ($hashlists as $hashlist) {
            if ($hashlist->getIsSecret() > $this->agent->getIsTrusted()) {
                ...
                $this->sendErrorResponse(PActions::SEND_PROGRESS, "Unknown Error. The ExtAPI does not trust you with more information");
            }
        }
        ...
        $collection = (new MongoDB())->selectCollection('hashes');
        ...
        $crackedHashesSize = sizeof($crackedHashes);

        for ($i = 0; $i < $crackedHashesSize; $i++) {
            ...
            switch ($format) {
                case DHashlistFormat::PLAIN:
                    ...
                    if (sizeof($plainUpdates) > 0) {

                        $qF = new ContainFilter(Hash::HASH_ID, $crackHashes);

                        /* Original Hashtopolis code
                        ...
                        */
                        ...
                        //Remove hashes from DB when cracked
