...
if (isset($_GET['new'])) {
  ...
  UI::add('pageTitle', "Create new ExtAPI Group");
}
  ...
  if ($group == null) {
    ...
    UI::add('pageTitle', "Details of ExtAPI Group " . htmlentities($group->getName(), ENT_QUOTES, "UTF-8"));
  }
}
  ...
  UI::add('pageTitle', "Create new ExtAPI key");
}
else if (isset($_GET['keyId'])) {
  $key = Factory::getApiKeyFactory()->get($_GET['keyId']);
  if ($key == null) {
    UI::printError(UI::ERROR, "Invalid ExtAPI key ID!");
  }
  if (MASK_API_KEYS) {
    $userID = Login::getInstance()->getUserID();
  ...
  UI::add('users', $users);
  UI::add('groups', Factory::getApiGroupFactory()->filter([]));
  UI::add('pageTitle', "Edit ExtAPI key");
  ...
  UI::add('pageTitle', "ExtApi Groups");
}
